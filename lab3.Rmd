...
g <- function(x, mean=0, sd=1){ #normal distribution for the enveloppe
  return(dnorm(x, mean, sd))
}
#Determining proper value of a for the enveloppe
x <- seq(-2, 2, 0.01)
plot(x, f(x), col = "red", type = "l", ylim = c(0, 1.5))
lines(x, g(x)/0.5, col = "blue")
lines(x, g(x)/0.4, col = "green")
lines(x, g(x)/0.3, col = "orange")
#add legend
legend("topright", legend = c("f(x)", "g(x)/0.5", "g(x)/0.4", "g(x)/0.3"), col = c("red", "blue", "green", "orange"), lty = 1, cex = 0.8)
...
```
We chose a simple normale distribution for the envelope. From this plot it can be seen that a value of a = 0.4 is not quite right therefore a = 0.35 is a good choice for the envelope.
...
par(mfrow=c(1,3))
hist(a, main="Rejection sampling", xlab="x", xlim=c(-2,2))
