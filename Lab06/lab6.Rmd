...
# EM algorithm
get_Q <- function(lambda_k){ #Given lambda and lambda_k, returns Q(lambda, lambda_k) to be optimised
  Q <- function(lambda){
    n*log(lambda) - lambda*n*mean(x) + m*log(lambda) -lambda*sum((1/lambda_k)-(c/(exp(lambda_k*c)-1))) - sum(log(1-exp(-lambda*c)))
  }
  return(Q)
}
