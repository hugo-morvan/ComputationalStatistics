...
$$ = E[n \log\lambda-\lambda \sum_{i=1}^n x_i + m \log \lambda - \lambda \sum_{j=1}^m y_j - \sum_{j=1}^m \log(1-e^{- \lambda c_j})]$$ 

$$ = n \log\lambda-\lambda E[\sum_{i=1}^n x_i] + m \log \lambda - \lambda E[\sum_{j=1}^m y_j] - \sum_{j=1}^m \log(1-e^{- \lambda c_j})$$.
...
$$Q(\lambda, \lambda^k) = n \log\lambda- \lambda n\bar{x} + m \log \lambda - \lambda \sum_{j=1}^m (\frac{1}{\lambda^{(k)}}-\frac{c_j}{e^{\lambda^{(k)} c_j}-1}) - \sum_{j=1}^m \log(1-e^{- \lambda c_j})$$ 
...
lambda_hat <- lambda_ks[length(lambda_ks)]
...
for(i in 1:B){
  ...
  bootstrap_data <- rexp(1000, rate = lambda_hat)
  #Sample from this data a number of samples = to the number of censored data un the original data (189)
  bootstrap_censored <- sample(bootstrap_data, 189)
  
  #Censor them 
  for (j in 1:length(bootstrap_censored)){
    bootstrap_censored[j] <- runif(1, min = 0, max = bootstrap_censored[j])
  }
  bootstrap_uncensored <- cbind(bootstrap_data[1:811], rep(1,811))
  bootstrap_censored <- cbind(bootstrap_censored, rep(2,189))
  combined_bootstrap_data <- rbind(bootstrap_uncensored, bootstrap_censored)
  
  ##################### Parameters #####################
  # uncensored and censored number of observations
  n <- length(which(combined_bootstrap_data[,2]==1))
  m <- length(which(combined_bootstrap_data[,2]==2))
  
  # uncensored and censored observations
  x <- as.numeric(combined_bootstrap_data[which(combined_bootstrap_data[,2]==1),1]) #Uncensored observations
  c <- as.numeric(combined_bootstrap_data[which(combined_bootstrap_data[,2]==2),1]) #Censored observations
  
  ####################### MLE #######################
  MLE_lambda_hat[i] <- 1/mean(x)
...
# Variances
cat("Variance of MLE_lambda_hat: ", var(MLE_lambda_hat), "\n")
cat("Variance of EM_lambda_hat: ", var(EM_lambda_hat), "\n")

```

The histograms for the two methods look similar. Both methods seem unbiased. The variance is `r var(MLE_lambda_hat)` for the MLE and `r var(EM_lambda_hat)` for the EM algorithm.

# Sources

Al-Athari, F. M. (2008). Estimation of the mean of truncated exponential distribution. Journal of Mathematics and Statistics, 4(4), 284–288. https://doi.org/10.3844/jmssp.2008.284.288
