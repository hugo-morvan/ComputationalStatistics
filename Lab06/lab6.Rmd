...
$$F(x,\lambda) =  P(X\leq x)= 1 - e^{-\lambda x}$$ for $x \geq 0$ and $\lambda > 0$.
The Truncated Exponential Distribution Function truncated on the left at c has the following density function:
$$f(y|c,\lambda) = \frac{\lambda e^{-\lambda y}}{1 -\lambda e^{-\lambda c}}$$ for $\lambda > 0$ and $0 < y \leq c$. (Source Al-Athari 2008)
...
since $E[\sum_{i=1}^n x_i] = n/\lambda$ and $E[\sum_{j=1}^m y_j] = m*(\frac{1}{\lambda^k}-\frac{c}{e^{\lambda c}-1}$ (Source : Al-Athari 2008).

where $\lambda^k$ is the current estimate of $\lambda$.
