...
fitness <- function(layout, fitness_function, n){
  ...
  if(fitness_function == "binary"){
      for(queen in 1:length(my_list)){
        for(other_queen in queen:length(my_list)){
          if(are_attacking_eachother(queen, other_queen)){
            fitness <- 1
          }
        } 
      }
...
vizualize_board(layout2, 4)
fitness(layout = layout0, fitness_function = "binary", 4)
```
