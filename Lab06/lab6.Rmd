...
darwin_pairs <- function(n_generations, board_size, n_population, mutprob, fitness_function){
  ###### Initialization ######
  ...
  return(n_attacking_queens)
}
res_pairs <- darwin_pairs(n_generations = 100, board_size= 4, n_population = 10, mutprob = 0.1, fitness_function = "num_safe")
```

```{r}
plot(res_pairs, type="l", xlab="Generation", ylab="Number of attacking queens")
```

```{r Binary Representation}
...
#The layout should be a list containing vectors of binary digits
#a <- list(c(0,0,1), c(1,1,1))
...
darwin_bin <- function(n_generations, board_size, n_population, mutprob, fitness_function){
...
```{r}
res <- darwin_bin(50, 4, 10, 0.1, "num_safe")
#res$n_attacking_queens
```

```{r}
plot(res$n_attacking_queens, type="l", xlab="Generation", ylab="Number of attacking queens")
```

```{r}
vizualize_board(res$solutions[[5]], 4)
length(res$solutions)
```

```{r Singles Representation}
...
darwin_singles <- function(n_generations, board_size, n_population, mutprob, fitness_function){
...
```{r}
res_sin <- darwin_singles(100, 4, 100, 0.1, "num_safe")
```
