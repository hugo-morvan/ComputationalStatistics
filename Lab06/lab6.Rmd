...
get_Q <- function(lambda){
  n * log(lambda) - n + (m * log(lambda)) - lambda * m * (1/lambda) - 
...
lambdas <- lambda # vector of lambda values, starting with lambda_0
L <- c(-Inf) # loglik vector for stopping condition

####################### EM Algorithm ##############################

while(k < max_k){
  L <- c(L, get_Q(lambdas[k]))
  if(abs(L[k+1] - L[k]) < epsilon ){
    print(paste("EM algorithm stopped at lambda =", lambda, "after", k, "iterations"))
    break
  }
  lambda <- optim(lambda, get_Q, method="BFGS", control = list(fnscale=-1))[[1]]
...
for(i in 1:1000){
  ...
  while(k < k_max){
    L2 <- c(L,get_Q(lambda)) # loglik
    if(abs(L2[k+1] - L2[k]) < epsilon){
      print(paste("EM algorithm stopped at lambda =", lambda, "after", 
                  k, "iterations"))
      break
    }
    lambda <- optim(lambda, get_Q, method="Brent")[[1]]
    k <- k +1
